...
This library runs behind Clientlog and Popstory -- two projects we've built with distinct route profiles and authorizer configurations, both defined in Python and deployed via the same Terraform pattern.
Marigold used the same pattern until its move to a self-hosted deployment; the library remains the right tool for anything staying on AWS API Gateway.
